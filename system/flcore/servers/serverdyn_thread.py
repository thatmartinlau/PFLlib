...
import time
import torch
from flcore.clients.clientdyn_thread import clientDynThread
from flcore.servers.serverbase import Server
from threading import Thread, Lock


class FedDynThread(Server):
    def __init__(self, args, times):
        super().__init__(args, times)

        # select slow clients
        self.set_slow_clients()
        self.set_clients(clientDynThread)
    ...
    def train(self):
        ...
        if self.num_new_clients > 0:
            ...
            self.set_new_clients(clientDynThread)
            print(f"\n-------------Fine tuning round-------------")
    ...
    def update_server_state(self):
        ...
        with self.cuda_lock:
            # Move global model and server state to GPU
            self.global_model = self.global_model.to('cuda:0')
            self.server_state = self.server_state.to('cuda:0')
            
            # Create model_delta on GPU
            model_delta = copy.deepcopy(self.uploaded_models[0]).to('cuda:0')
            for param in model_delta.parameters():
                param.data = torch.zeros_like(param.data, device='cuda:0')

            # Process each client model
            for client_model in self.uploaded_models:
                # Move client model to GPU
